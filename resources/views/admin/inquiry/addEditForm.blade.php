...
<div class="row">
    ...
    <div class="card" >
        ...
        <div class="row">
            ...
            <div class="col-md-7">
                <h3 class="card-title">    
                    <a href="{{ route('admin.inquiry.index') }}" class="btn btn-info float-end mt-2">
                ...
                <div class="col-md-8 regCustomer">
                        <!-- registered customer -->
							...
							@if(isset($inquiry->id) && $inquiry->id!='' )
								@php
								$accountId= $inquiry->account?->id;
								$priceGroup=$inquiry->account?->priceGroup;
								@endphp
								<form role="form" action="{{ route('admin.inquiry.update', $inquiry->id) }}" method="post" enctype="multipart/form-data" autocomplete="off" id="billingForm">
								 @method('PUT')
							@else
								<form role="form" action="{{ route('admin.inquiry.store') }}" method="post" enctype="multipart/form-data" autocomplete="off" id="billingForm">
								@php $accountId=""; $priceGroup=""; @endphp
							@endif
                                    ...
                                    @endphp
                                        @if($inquiry->account)
                                            <span class="pull-left">
                                                <strong>{{$inquiry->account?->name}}</strong>
                                                <br>{{$inquiry->account?->phone.','.$inquiry->account?->phone2.' ('.$inquiry->account?->email.')'}}<br>
                                                {{ $inquiry->account?->contactPerson }}
                                                <br>Address:-{{$inquiry->account?->address.' '.$inquiry->account?->citydata?->name.' '.$inquiry->account?->statedata?->name}}
                                            </span>
                                        @else 
                                        @endif
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <div class="form-group">
                                    ...
                                    <label>Product Search QRCODE <span class="text-danger">*</span>
                                    <input type="checkbox" name="catall" id="catall">Similar</label><i class="fa fa-camera" id="scan-btn"></i>
                                    <input id="prodQrData" name="prodQrData"  class="form-control" placeholder="Search By QRcode">
                                </div>

                                <div class="modal"  id="qr-modal" tabindex="-1">
                                    <div class="modal-dialog">
                                        ...
                                        <!-- QR reader will be displayed here -->
...
<script>
   ...
   function qrInputProdData(){
    ...
    $('#prodQrData').bind('change keyup keydown',function (e) {
...
function DeleteRow(obj)
    {
      ...
      if(confirm('Are you sure you want to delete this item Permanently?'))
        {
            if($oldID>=1){
                $.getJSON("{{url('admin/action')}}/"+'sale-inquery-itemRemove/'+$oldID, function(data){
...
messages: {
    ...
    "inqfor":{
        required:function(){
            toastr.error('Please choose Inquery Type/Imnquery For')
        }
