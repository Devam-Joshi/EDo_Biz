        ...
        <div class="card-body">
            <div class="mb-2 text-right">
                <div class="wd-sl-modalbtn">
                    <a href="{{route('admin.inquiry.create')}}">
                            ...
                            @if(!empty($inquiry))
                                @foreach($inquiry as $key => $pur)
                                        <tr>
										   ...
										   <td></td>
                                            <td class="action">
                                                <a href="{{ route('admin.inquiry.edit', $pur->id) }}" class="text-primary" target="_blank"><i class="fa fa-pen"></i></a> 
												 <a href="{{url('admin/convert-inquiry-saleorder/'.$pur->id)}}" class="text-secondary" target="_blank" title="Sale order to bill">
                                                ...
                                                <a href="{{ route('admin.inquiry.edit', $pur->id) }}" class="text-info" title="Edit order">
                                                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                                </a>
                                                <span class="text-danger" type="button" onclick="deleteItem({{ $pur->id }})" title="Delete order">
                                                    <i class="fa fa-trash" aria-hidden="true"></i>
                                                </span>
                                                <form id="delete-form-{{ $pur->id }}" action="{{ route('admin.inquiry.destroy', $pur->id) }}" method="post"
                                                      style="display:none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
